...
# df['Parsed_Categories'] = df['Categories_and_confidence'].apply(extract_categories)
# df['First_Category'] = df['Parsed_Categories'].apply(lambda x: x[0] if x else 'Unknown')
...
def save_plot(filename, folder_path='c:/NTSB/output'):
    """
...
def plot_accidents_per_year(df):
    ...
    # Adjust the bar width here; for example, using 0.8 for slightly wider bars
    ax = yearly_category_counts.plot(kind='bar', stacked=True, color=[category_color_map.get(cat) for cat in yearly_category_counts.columns], figsize=(12, 7), width=0.9)
    ...
    plt.legend(handles=legend_patches, title='Categories', fontsize=12)
...
def plot_accidents_per_month(df):
    ...
    # Plot stacked bar chart using the category colors
    ax = monthly_category_counts.plot(kind='bar', stacked=True, color=[category_color_map.get(cat) for cat in monthly_category_counts.columns], figsize=(12, 7), width=0.8)
...
if __name__ == "__main__":
    try:
        ...
        # Generate the map and get the category color mapping
